...
function EditableSection({ module, children }: { module: string; children: ReactNode }) {
...
function TimeRange({ time }: { time?: [string?, string?] }) {
  if (!time) return null;
...
function getTitle(config: ResumeConfig, key: string, fallback: string) {
  return config.titleNameMap?.[key] ?? fallback;
}

function isHidden(config: ResumeConfig, key: string) {
