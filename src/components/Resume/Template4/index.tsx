              ...
              <div key={proj.id} className="mb-4">
                <div className="flex items-baseline justify-between">
                  <div className="flex items-baseline gap-2">
                    ...
                    {proj.projectRole && (
                      <span className="rounded px-1.5 py-0.5 text-xs" style={{ backgroundColor: 'color-mix(in srgb, var(--resume-tag) 20%, transparent)', color: 'var(--resume-tag)' }}>
                        {proj.projectRole}
                      </span>
                    )}
                  </div>
            ...
            {config.educationList.map((edu) => (
              <div key={edu.id} className="mb-2 flex items-baseline justify-between">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs text-gray-500">{edu.major}{edu.academicDegree && ` · ${edu.academicDegree}`}</span>
                </div>
...
export function Template4({ config }: { config: ResumeConfig }) {
  const { profile, avatar } = config;
  const { t } = useTranslation();
  const age = calculateAge(profile?.birthday);
  const layout = getEffectiveLayout('template4', config.moduleLayout);
